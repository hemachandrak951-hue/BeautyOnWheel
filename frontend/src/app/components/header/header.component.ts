    ...
    <header class="sticky top-0 z-50 w-full glassmorphism shadow-premium border-b border-slate-100">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-4">
          <!-- Live Mode Toggle (Visual Flag Indicator for Mocking/Spring Boot) -->
          <button (click)="toggleMockMode()" 
                  [title]="isMock ? 'Currently in Offline Simulator Mode. Click to switch.' : 'Currently connected to Spring Boot API. Click to switch.'"
                  class="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all duration-300 border border-dashed"
                  [ngClass]="isMock ? 'border-amber-400 bg-amber-50 text-amber-700' : 'border-emerald-400 bg-emerald-50 text-emerald-700'">
            <span class="relative flex h-2 w-2">
              <span [ngClass]="isMock ? 'bg-amber-500' : 'bg-emerald-500 animate-ping'" class="absolute inline-flex h-full w-full rounded-full opacity-75"></span>
              <span [ngClass]="isMock ? 'bg-amber-500' : 'bg-emerald-500'" class="relative inline-flex rounded-full h-2 w-2"></span>
            </span>
            {{ isMock ? 'Offline Simulator' : 'Spring Boot API' }}
          </button>
            ...
            <div *ngIf="showUserDropdown" 
              ...
              <div class="p-4 border-b border-slate-50 bg-slate-50/50">
                ...
                <div class="mt-2.5 flex items-center gap-1.5">
                  ...
                  <span *ngIf="isMock" class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 uppercase">
                    Simulated
                  </span>
                </div>
              </div>
              ...
              <!-- Menu Items List -->
              <div class="py-1">
                <button (click)="mockAction('My Bookings')" 
                        class="w-full text-left px-4 py-2.5 hover:bg-slate-50 text-sm font-medium text-slate-700 flex items-center gap-2.5 transition-colors">
                  <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path>
                  </svg>
                  My Active Bookings
                </button>
                <button (click)="mockAction('Wallet & Coupons')" 
                        class="w-full text-left px-4 py-2.5 hover:bg-slate-50 text-sm font-medium text-slate-700 flex items-center gap-2.5 transition-colors">
                  <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Toggle Mode directly inside menu -->
                <button (click)="toggleMockMode()" 
                        class="w-full text-left px-4 py-2.5 hover:bg-slate-50 text-sm font-medium text-slate-700 flex items-center gap-2.5 transition-colors border-t border-slate-50">
                  <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4"></path>
                  </svg>
                  Toggle Core Engine Mode
                </button>
              </div>
              ...
              <div class="border-t border-slate-100 bg-slate-50 p-2">
                <button (click)="logout()" 
                        class="w-full py-2.5 px-3 rounded-xl hover:bg-rose-50 text-sm font-bold text-rose-600 hover:text-rose-700 flex items-center justify-center gap-2 transition-all">
...
export class HeaderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Set up mock status
    this.authService.isMockMode$.subscribe(mode => {
      this.isMock = mode;
    });
  }
  ...
  public toggleMockMode(): void {
    const nextState = !this.isMock;
    this.authService.setMockMode(nextState);
    this.isMock = nextState;
    this.showUserDropdown = false;
    this.toastService.showInfo(
      nextState 
        ? 'Switched core to Offline simulation mode. REST operations will use RxJS delay streaming.'
        : 'Switched core to Live Spring Boot REST mode. API requests will point to http://localhost:8080.'
    );
  }
  ...
  public triggerSearch(): void {
    if (this.searchQuery.trim()) {
      ...
      // Scroll to category grid or broadcast search
    }
  }
